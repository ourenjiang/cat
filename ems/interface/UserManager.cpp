...
try
{
    ...
    UserDb << "SELECT GRADE, PHONE, EMAIL FROM USER WHERE NAME = ?;"
    ...
    //记录操作日志
    base::OperationRecord::insertRecord("成功", "查询用户信息", "用户管理", usernameAuth);
    ...
    if(authException){
        base::OperationRecord::insertRecord("失败", authException->what(), "用户管理", authException->username());
    }
...
try
{
    ...
    UserDb << "SELECT GRADE FROM USER WHERE NAME = ? AND PASSWORD = ?;"
    ...
    UserDb << "SELECT NAME, PASSWORD, GRADE, PHONE, EMAIL FROM USER WHERE GRADE >= ?;"
        << grade 
...
try
{
    ...
    if(!authGrade.has_value())
        throw std::runtime_error("auth failed");
    if(authGrade.value() > 0)// 只有root用户才有权限
    ...
    //记录正常操作日志
    base::OperationRecord::insertRecord("成功", "添加用户", "用户管理", usernameAuth);
...
try
{
    ...
    UserDb << "SELECT PASSWORD, GRADE, PHONE, EMAIL FROM USER WHERE NAME = ?;" << username
    ...
    // 修改
    UserDb << "UPDATE USER SET PASSWORD=?, GRADE=?, PHONE=?, EMAIL=? WHERE NAME = ?;"
                << passwordNew << gradeNew << phoneNew << emailNew << username;

    //记录操作日志
    base::OperationRecord::insertRecord("成功", "修改用户信息", "用户管理", usernameAuth);
...
try
{
    ...
    //记录操作日志
    base::OperationRecord::insertRecord("成功", "删除用户 " + usernameOld, "用户管理", usernameAuth);
