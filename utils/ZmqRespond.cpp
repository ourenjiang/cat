#include "ZmqRespond.h"
#include <unistd.h>
#include <iostream>

using namespace ems;

ZmqRespond::ZmqRespond(const string _addr)
    : addr_(_addr)
    , context_(1)
{
    init();
...
bool ZmqRespond::send(const vector<byte>& data)
{
...
std::optional<vector<byte>> ZmqRespond::recv()
{
